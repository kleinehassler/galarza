...
CREATE VIEW vsecudoc AS 
 SELECT DISTINCT e.idsecudoc, e.iddocu, e.idautosri, e.nota, d.cretri, d.ivaxitem, d.lispre, a.numauto, a.fcaducidad, 
 	...
 	f.idimage, d.isdevo, d.istransfer, d.numregdet, d.tipliq, d.tipoambi, u.isdocele
   FROM secudoc e
...
CREATE  VIEW vdocuments AS 
 ...
 x.descripda AS tipdocextraj, s.issocio, d.clave, d.autoriza, s.smail,
 d.linkser, d.linknum, d.linkfec, d.coddocmod, d.isenvride
   FROM documents d
...
CREATE VIEW vplancta AS
	...
	p.idplancuenta, p.idcuenta, p.pdocode, p.auxsuj, 
    CASE WHEN (p.plannivel = 1) THEN substr((p.plancod)::text, 2, 1) 
    	 WHEN (p.plannivel = 2) THEN (substr((p.plancod)::text, 2, 1) || substr((p.plancod)::text, 4, 1)) 
    	 WHEN (p.plannivel = 3) THEN (((substr((p.plancod)::text, 2, 1) || substr((p.plancod)::text, 4, 1)) || '.'::text) || substr((p.plancod)::text, 5, 2)) 
    	 WHEN (p.plannivel = 4) THEN (((((substr((p.plancod)::text, 2, 1) || substr((p.plancod)::text, 4, 1)) || '.'::text) || substr((p.plancod)::text, 5, 2)) || '.'::text) || substr((p.plancod)::text, 7, 2)) 
    	 WHEN (p.plannivel = 5) THEN (((((((substr((p.plancod)::text, 2, 1) || substr((p.plancod)::text, 4, 1)) || '.'::text) || substr((p.plancod)::text, 5, 2)) || '.'::text) || substr((p.plancod)::text, 7, 2)) || '.'::text) || substr((p.plancod)::text, 9, 2)) 
    	 WHEN (p.plannivel = 6) THEN (((((((((substr((p.plancod)::text, 2, 1) || substr((p.plancod)::text, 4, 1)) || '.'::text) || substr((p.plancod)::text, 5, 2)) || '.'::text) || substr((p.plancod)::text, 7, 2)) || '.'::text) || substr((p.plancod)::text, 9, 2)) || '.'::text) || substr((p.plancod)::text, 11, 2)) 
    	 ELSE ((((((((((substr((p.plancod)::text, 2, 1) || substr((p.plancod)::text, 4, 1)) || '.'::text) || substr((p.plancod)::text, 5, 2)) || '.'::text) || substr((p.plancod)::text, 7, 2)) || '.'::text) || substr((p.plancod)::text, 9, 2)) || substr((p.plancod)::text, 11, 2)) || '.'::text) || substr((p.plancod)::text, 13, 2)) END AS plancod
...
CREATE  VIEW vdocui AS 
 SELECT t.code, i.iditem, i.icode, i.iname, i.marca, i.grupo, i.subgrupo, i.itag, i.genero, i.itipo, i.iname AS auxiliar, 
 	...
 	i.pvp, i.factorc, i.numeconver, i.denoconver, i.isiva as isivai, i.ubipro, i.ubica2, t.observ
   FROM detadoci t
...
CREATE  VIEW vdocumentg AS 
 SELECT DISTINCT d.code, s.sname AS nombre, v.nomdoc AS documento, d.num, s.scode AS cod_persona, s.ssri AS sri, 
 	...
 	g.sname as garante, d.chofer, d.nrec, d.autoriza, d.clave 
   FROM documents d
...
CREATE  VIEW vdocusmall AS 
 SELECT DISTINCT d.code, d.userstore, d.usercaja, d.num, d.cliente, d.fecha, d.fto, d.pdocode, d.idsecudoc, d.seller, 
 		...
 		0 as idprin, 0 as idgar, d.sujter, v.ajucon, d.idagen,  v.tipoambi,
 		case when u.tipocontri=1 then 'ESPECIAL'
 		 	 when u.tipocontri=2 then 'EXPORTADOR'
 		     when u.tipocontri=3 then 'OTROS'
 		     else
 		     	'Obli.LL.Con.' end::char(10)  as tipocontri   
   FROM documents d
...
CREATE OR REPLACE VIEW vdocuret AS 
 SELECT d.code, d.fecharet AS fecha, d.numsecue1 AS numero, d.montoret, d.numserie1 AS seriedoc, 
	d.numsecue1 AS numsecdoc, d.numsecue2, d.secueret, d.autoret, d.fcaducret, d.numserie2, d.serieret, 
	d.numauto2 as numautadq, d.fecha2 as fecdocadq, fcaducidad2 as feccadadq, 
	s.descrip AS tipdoc, u.fecha AS feccom, u.sname, u.sruc, u.scedula, u.saddr, u.sciudad, u.concepto,
	u.stelf, r.rubname, r.rubtab, r.valcal, c.valor, c.basecal AS monto, v.nomdoc, i.codigo as codigoret,
	d.fimpresion, z.isdocele, d.idsecudoc, z.reporte, d.clave, d.autoriza
  FROM aneiva d
   LEFT JOIN vdocuments u ON d.code = u.code
   LEFT JOIN cobros c ON c.code = d.code
...
CREATE  VIEW vpreciosv AS 
 SELECT c.descripda AS clientes, i.icode, i.iname, i.artcosto, i.artgrupo, i.artmarca, i.itag, p.preciov, p.idlista, 
 	...
 	u.descripda as unidad, s.descripda AS subgrupo, i.artsgrupo, i.costopro
   FROM preciosv p
...
CREATE VIEW vsujetos AS
    ...
    v.idsujeto as idven, v.sname as vendedor, x.descripda as ntipdocextraj 
...
CREATE OR REPLACE VIEW vdocxml1 AS  SELECT DISTINCT d.idugra, d.fecgra, d.code, d.succode, d.depcode, d.usercaja, 
    d.userstore, d.num, d.idsecudoc, d.cliente, d.fecha, d.pdocode, d.ffrom, 
    d.fto, d.seller, d.casher, d.linkdoc, d.sujter, d.auxitem, d.duracion, 
    d.taza, d.valcuota, d.numcuota, d.numcajas, d.poriva, d.descuconiva, 
    d.descusiniva, d.recargo, d.recargo2, d.recargo3, d.flete, d.subconiva, 
    z.tipitem, d.chofer, f.sname AS chname, f.scedula AS chcedula, 
    f.sruc AS chruc, d.nrec, d.comenanu, d.subsiniva, d.subtotal, d.ivavalor, 
    d.ivabienes, d.icevalor, d.ipsvalor, d.montototal, d.saldo, d.tmpsaldo, 
    d.isguia, d.isupdate, d.islock, d.isprint, d.isaccount, d.isbodega, 
    d.ispagado, d.isanulado, d.isautorizado, d.isreversado, d.concepto, 
    k.afecha AS feccon, b.descripda AS bodega, z.tipsaldo, z.iddocu, z.nomdoc, 
    z.iscero, z.dtag, z.swaneiva, z.tipnum, z.swkar, z.swconta, z.idsritabla, 
    z.ajucon, d.serie, d.numauto, d.fcaducidad, s.scel, s.stelf, s.saddr, 
    s.saddr2, s.scode, s.ssri, s.sname, s.sruc, s.scedula, s.tipocontri,
    w.descripda AS sciudad, s.spasaporte, j.sname AS vendedor, 
    u.sname AS nombreu, u.observ, monedas.moname AS moneda, d.pflag, d.dflag, 
    d.idws, q.reporte, t.scode AS codter, t.sname AS nombret, z.isdevo, 
    z.istransfer, z.documentos, r.sname AS recaudador, c2.descripda AS ciudad2, 
    s.diasplz, a.sname AS cajero, v.descripda AS provincia, s.calific, z.cretri, 
    x.descripda AS tipdocextraj, s.issocio, s.smail, d.isxmlenv, d.isxmlres, 
    d.isxmlcon, z.tipoambi, d.placa, d.clave, d.autoriza,
    d.linkser, d.linknum, d.linkfec, d.coddocmod, d.isenvride
   FROM documents d
   LEFT JOIN detagru b ON d.userstore = b.iddato
   LEFT JOIN asientosdia k ON d.code = k.code
   LEFT JOIN vsecudoc z ON d.idsecudoc = z.idsecudoc
   LEFT JOIN docuse v1 ON z.iddocu = v1.iddocu
   LEFT JOIN imagenrep q ON z.idimage = q.idimage
   LEFT JOIN sujetos u ON d.idugra = u.idsujeto
   LEFT JOIN periodos o ON d.pdocode = o.pdocode
   LEFT JOIN monedas ON o.moneda = monedas.mocode
   ...
   LEFT JOIN sujetos j ON d.seller = j.idsujeto
   LEFT JOIN sujetos r ON d.casher = r.idsujeto
   LEFT JOIN detagru w ON s.sciudad = w.iddato
   LEFT JOIN detagru c2 ON s.sciudad2 = c2.iddato
   LEFT JOIN detagru v ON s.sprov = v.iddato
   LEFT JOIN sujetos a ON d.usercaja = a.idsujeto
   LEFT JOIN sujetos t ON d.sujter = t.idsujeto
   LEFT JOIN sujetos f ON d.chofer = f.idsujeto
   LEFT JOIN detagru x ON s.tipdocextraj = x.iddato
   WHERE not d.isanulado and v1.iddocu in (select distinct d1.iddocu from gdoc g1, dgdoc t1, docuse d1 
   where g1.tag='DOCELE' and g1.idgdoc=t1.idgdoc and t1.dtag=d1.dtag );

ALTER TABLE vdocxml1 OWNER TO postgres;


CREATE OR REPLACE VIEW vdocxml2 as
 SELECT d.code, d.fecharet AS fecha, d.numsecue1 AS numero, d.montoret, 
    d.numserie1 AS seriedoc, d.numsecue1 AS numsecdoc, d.numsecue2, d.serieret, 
    d.secueret, d.autoret, d.fcaducret, d.numserie2, d.numauto2 AS numautadq, 
    d.fecha2 AS fecdocadq, d.fcaducidad2 AS feccadadq, s.descrip AS tipdoc, 
    u.fecha AS feccom, u.sname, u.sruc, u.scedula, u.spasaporte, u.saddr, 
    u.saddr2, u.sciudad, x.smail, u.stelf, u.scel, r.rubname, r.rubtab, u.cliente,
    r.valcal, c.valor, c.basecal AS monto, v.nomdoc, i.codigo AS codigoret, 
    d.fimpresion, z.isdocele, z.tipoambi, r.rubtab2, 
    to_char(s.codigo::smallint::integer, 'FM09'::text) AS tipdocsustento, 
    d.clave, d.autoriza, d.isxmlres, d.isxmlenv, d.isxmlcon, d.isenvrider
   FROM aneiva d
   LEFT JOIN vdocuments u ON d.code = u.code
   LEFT JOIN cobros c ON c.code = d.code
   LEFT JOIN sritabla i ON c.idsritabla = i.idsritabla
  ...
  WHERE r.isdocret AND z.isdocele and not u.isanulado;

ALTER TABLE vdocxml2 OWNER TO postgres;

CREATE OR REPLACE VIEW vkardex AS 
SELECT a.iditem, a.fecha, a.punitario, a.costoprom, qty, bodega
   FROM (         SELECT s.iditem, p.fecini AS fecha, s.cospro AS punitario, s.precio AS costoprom, qtyini as qty, bodega
                   FROM saldosi s LEFT JOIN periodos p ON s.fecsaldo = p.fecini
				  left join saldosib b on s.idsaldosi=b.idsaldosi
              where not fecini isnull
        UNION all
                 SELECT t.iditem, d.fecha, ((t.punitario*t.qty - t.descuento) / t.qty) AS punitario, t.costoprom AS costoprom, 
			case when tipsaldo=1 then qty*(-1) else qty end as qty, userstore as bodega
                   FROM detadoci t
              LEFT JOIN vdocusmall d ON t.code = d.code
             WHERE case when swconta then isaccount else 't' end AND NOT d.isanulado AND (d.iddocu IN ( SELECT DISTINCT d.iddocu
                      FROM gdoc g, dgdoc t, docuse d
                     WHERE g.tag::text = 'KARKAR'::text AND g.idgdoc = t.idgdoc AND t.dtag::text = d.dtag::text)) ) a;

ALTER TABLE vkardex OWNER TO postgres;

CREATE OR REPLACE VIEW vadqing AS 
 SELECT a.iditem, a.fecha, a.punitario, a.costoprom
   FROM (         SELECT s.iditem, p.fecini AS fecha, s.cospro AS punitario, s.precio AS costoprom
                   FROM saldosi s
              LEFT JOIN periodos p ON s.fecsaldo = p.fecini
              where not fecini isnull
        UNION all
                 SELECT t.iditem, d.fecha, max(t.punitario - t.descuento / t.qty) AS punitario, max(t.costoprom) AS costoprom
                   FROM detadoci t
              LEFT JOIN vdocusmall d ON t.code = d.code
             WHERE d.isaccount AND NOT d.isanulado AND (d.iddocu IN ( SELECT DISTINCT d.iddocu
                      FROM gdoc g, dgdoc t, docuse d
                     WHERE g.tag::text = 'ADQING'::text AND g.idgdoc = t.idgdoc AND t.dtag::text = d.dtag::text))
             GROUP BY t.iditem, d.fecha) a;

ALTER TABLE vadqing OWNER TO postgres;
